...
public class Parser {
    ...
    public CompUnitNode getCompUnit() {
        return compUnitNode;
    }

    public void parse() {
        if (compUnitNode != null) { compUnitNode = (CompUnitNode) parseCompUnit(); }
    }
    ...
    private Node parseConstDecl() {
        ...
        if (curToken.type() != TokenType.SEMICN) {
            unread();
            Recoder.addErrorMessage(ErrorType.missingSEMICN, lineno);
    ...
    private Node parseConstDef() {
        ...
        if (curToken.type() != TokenType.RBRACK) {
            unread();
            Recoder.addErrorMessage(ErrorType.missingRBRACK, lineno);
    ...
    private Node parseConstInitVal() {
        ...
        if (curToken.type() == TokenType.STRCON) {
            ...
            unread();
            return new ConstInitValNode(SyntaxCompType.FAIL, children);
        }
    ...
    private Node parseFuncDef() {
        ...
        if (curToken.type() != TokenType.RPARENT) {
            unread();
            Recoder.addErrorMessage(ErrorType.missingRPARENT, lineno);
